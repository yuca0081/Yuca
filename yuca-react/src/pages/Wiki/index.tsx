...
import { Plus, Folder, FolderOpen, FileText, Search, ChevronRight, ChevronDown, BookOpen } from 'lucide-react'
...
const mockTree: WikiItem[] = []

export default function Wiki() {
  const [tree, setTree] = useState<WikiItem[]>(mockTree)
  const [selectedId, setSelectedId] = useState<number | null>(null)
  const [search, setSearch] = useState('')

  const toggleExpand = (id: number, items: WikiItem[]): WikiItem[] =>
    items.map(item => {
  ...
  const renderItem = (item: WikiItem, depth = 0) => (
    <div key={item.id}>
      <div
      ...
      <aside className="sidebar w-64 shrink-0 overflow-hidden flex flex-col">
        <div className="p-4">
          ...
          <div className="relative">
            ...
            <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索知识..." className={`pl-8 h-9 text-xs ${"bg-white border-2 border-foreground focus:border-[#FF6B35] rounded-none shadow-none"}`} />
          </div>
        </div>
        ...
        {selectedId ? (
          <ScrollArea className="flex-1 p-6">
            <article className="prose max-w-none">
              <h1 className="font-bold">知识条目</h1>
              <p className="text-[#6B5344]">选择条目查看内容</p>
            </article>
          </ScrollArea>
        ) : (
          <div className="flex-1 flex items-center justify-center text-[#6B5344]">
            <div className="text-center">
              ...
              <p>选择一个知识条目</p>
              <p className="text-sm text-[#E8DDD4] mt-1">或从左侧创建新条目</p>
            </div>
          </div>
        )}
      </main>
