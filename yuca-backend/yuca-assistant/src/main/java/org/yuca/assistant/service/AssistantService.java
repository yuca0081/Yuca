...
public class AssistantService {
     ...
     */
    @Transactional
    public void processChat(Long userId, org.yuca.assistant.dto.request.ChatRequest request, SseEmitter emitter, HttpServletResponse response) {
        ...
        try {
            ...
            List<AIMessage> aiMessages = buildAIMessages(historyMessages, request.getContent());
            ...
            // 发送完成事件
            sendSseEvent(emitter, response, new SseEvent.SseDoneEvent(assistantMessage.getId(), fullResponse.toString()));
            emitter.complete();

            // 生成标题（首次对话）
            if (session.getTitle() == null || session.getTitle().isEmpty()) {
                generateTitle(session, request.getContent(), fullResponse.toString());
            }
    ...
    private AssistantMessage saveAssistantMessage(Long sessionId, String content) {
        AssistantMessage message = AssistantMessage.builder()
            .sessionId(sessionId)
    ...
    private List<AIMessage> buildAIMessages(List<AssistantMessage> history, String newContent) {
        List<AIMessage> messages = new ArrayList<>();
        messages.add(AIMessage.system("你是一个智能助手，帮助用户解答问题和提供建议。请用简洁、友好的语言回复。"));
        ...
        messages.add(AIMessage.user(newContent));
        return messages;
    }

    private void generateTitle(AssistantSession session, String userMessage, String assistantMessage) {
        try {
            String prompt = String.format("为以下用户的提问提炼主题并生成一个一句话标题（注意：标题中不要含有“用户”等主语的描述，只是一段概括）：\n用户：%s", userMessage);

            List<AIMessage> messages = List.of(
                AIMessage.system("你是一个标题生成助手，只返回标题文字，不要任何解释。"),
